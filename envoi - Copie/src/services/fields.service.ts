...
export class FieldsService {
  ...
  private static writeFieldsDB(dbos: FieldDBO[]): void {
    try {
      ...
      //Log the internal error using LoggerService
      LoggerService.error(
        `[FieldsService] Failed to write to database: ${error}`,
      );
      throw new Error("Internal Error");
    }
  }

  private static getNewID(dbos: FieldDBO[]): number {
    let maxId = 0;
  ...
  static create(newField: NewFieldDTO): FieldDTO {
    ...
    LoggerService.info(
      `[FieldsService] Field created successfully with ID: ${newId}`,
    );
    return FieldsMapper.toDTO(newDbo);
  }
  static update(id: number, fieldData: FieldDTO): FieldDTO | undefined {
    LoggerService.info(
      `[FieldsService] Attempting to update field with ID: ${id}`,
    );
    const dbos = this.readFieldsDB();
    ...
    if (index === -1) {
      LoggerService.error(
        `[FieldsService] Update failed: Field ID ${id} not found`,
      );
