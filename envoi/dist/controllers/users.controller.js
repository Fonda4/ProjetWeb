...
import { isUserDTO } from "../utils/guards";
 ...
 *  */
usersController.get("/", AuthService.authorize, (req, res) => {
    LoggerService.info("[UsersController] GET /users called");
    const loggedInUser = req.user;
    if (!loggedInUser) {
        LoggerService.error("[UsersController] Unauthorized: Unauthenticated user access attempt");
    ...
    if (loggedInUser.role === EROLES.ADMIN) {
        LoggerService.info(`[UsersController] Admin ${loggedInUser.username} retrieved the full users list`);
    ...
    LoggerService.info(`[UsersController] User ${loggedInUser.username} retrieved the short users list`);
...
usersController.post("/", (req, res) => {
    ...
    if (!UsersService.isValidEmail(userData.email)) {
        LoggerService.error(`[UsersController] Bad Request: Invalid email format provided (${userData.email})`);
        return res.status(400).send("Bad Request: The email must contain an '@', a '.' and be at least 5 characters long.");
    }
    // Business Rule: Check for email uniqueness
    const existingUser = UsersService.getByEmail(userData.email);
    if (existingUser) {
        LoggerService.error(`[UsersController] Conflict: A user with email ${userData.email} already exists`);
        return res.status(409).send("Conflict: A user with this email already exists.");
    }
    // Happy Path: Call the service to create the user
    const newUser = UsersService.create(userData);
    LoggerService.info(`[UsersController] User ${newUser === null || newUser === void 0 ? void 0 : newUser.username} successfully created`);
...
usersController.get("/username/:username", (req, res) => {
    ...
    LoggerService.info(`[UsersController] GET /users/username/${req.params.username} called`);
    ...
    LoggerService.info(`[UsersController] User '${username}' successfully retrieved`);
 ...
 */
usersController.get("/email/:email", AuthService.authorize, (req, res) => {
    // 1. Log the incoming request
    LoggerService.info(`[UsersController] GET /users/email/${req.params.email} called`);
    // 2. Check permissions
    if (req.user.role !== EROLES.ADMIN && req.user.role !== EROLES.REFEREE) {
        LoggerService.error(`[UsersController] Forbidden: User ${req.user.username} is not Admin or Referee`);
    ...
    LoggerService.info(`[UsersController] User with email '${email}' successfully retrieved`);
    res.status(200).json(user);
});
/**
 * GET /users/:id
 ...
 * Auth: REQUIRED
 */
usersController.get("/:id", AuthService.authorize, (req, res) => {
    // 1. Log the incoming request
    LoggerService.info(`[UsersController] GET /users/${req.params.id} called`);
    const loggedInUser = req.user;
    // 2. Guard: Check authentication
    if (!loggedInUser) {
        LoggerService.error("[UsersController] Unauthorized: Unauthenticated user");
        return res.status(401).send("Unauthenticated user");
    }
    const id = Number(req.params.id);
    // 3. Guard: Check if the ID is a valid number
    if (!isNumber(id)) {
        LoggerService.error(`[UsersController] Bad Request: Invalid ID provided (${req.params.id})`);
    ...
    // 5. Handle failure: User not found
    if (!targetUserDBO) {
        LoggerService.error(`[UsersController] Not Found: User ${id} does not exist`);
        return res.status(404).send("Not Found: User does not exist");
    }
    // 6. Business Logic & Happy Path
    // Rule A: Admin OR user requesting their own data receives full DTO
    if (loggedInUser.role === EROLES.ADMIN || loggedInUser.id === id) {
        LoggerService.info(`[UsersController] Full data for user ${id} retrieved by ${loggedInUser.username}`);
        return res.status(200).json(UsersMapper.toDTO(targetUserDBO));
    }
    // Rule B: User requesting another user's data receives Short DTO
    else {
        LoggerService.info(`[UsersController] Short data for user ${id} retrieved by ${loggedInUser.username}`);
 ...
 * Auth: REQUIRED
 */
usersController.delete("/:id", AuthService.authorize, (req, res) => {
    LoggerService.info(`[UsersController] DELETE /users/${req.params.id} called`);
    const id = Number(req.params.id);
    ...
    const loggedInUser = req.user;
    if (!loggedInUser) {
        LoggerService.error("[UsersController] Unauthorized: Missing or invalid token");
        return res.status(401).send("Missing or invalid token");
    }
    const targetUser = UsersService.getById(id);
    if (!targetUser) {
        LoggerService.error(`[UsersController] Unauthorized: Target user ${id} is invalid or does not exist`);
        return res.status(401).send("Invalid User");
    }
    if (targetUser.role == EROLES.ADMIN) {
        LoggerService.error(`[UsersController] Forbidden: Attempt to delete an admin account (ID: ${id})`);
        return res.status(403).send("Invalid ID or attempt to delete an admin account");
    }
    if (loggedInUser.role !== EROLES.ADMIN && loggedInUser.id !== id) {
        LoggerService.error(`[UsersController] Forbidden: User ${loggedInUser.username} cannot delete another user`);
    ...
    if (!isDeleted) {
        LoggerService.error(`[UsersController] Not Found: User ${id} not found or cannot be deleted`);
        return res.status(404).send("Not Found: User not found or cannot be deleted");
    }
    // Happy Path
    LoggerService.info(`[UsersController] Success: User ${id} has been soft deleted by ${loggedInUser.username}`);
    res.status(200).send();
});
/**
 * PUT /users/:id
 ...
 * Auth: REQUIRED
 */
usersController.put("/:id", AuthService.authorize, (req, res) => {
    ...
    if (!isNumber(id)) {
        LoggerService.error(`[UsersController] Bad Request: Invalid ID (${req.params.id})`);
        return res.status(400).send("Bad Request: Invalid ID");
    }
    ...
    if (id !== bodyData.id) {
        LoggerService.error("[UsersController] Bad Request: Path ID and Body ID mismatch");
        return res.status(400).send("Bad Request: Path ID and Body ID mismatch");
    }
    if (!UsersService.isValidEmail(bodyData.email)) {
        LoggerService.error(`[UsersController] Bad Request: Invalid email format (${bodyData.email})`);
    ...
    if (existingUser && existingUser.id !== id) {
        LoggerService.error(`[UsersController] Conflict: Email ${bodyData.email} already exists`);
        return res.status(409).send("Conflict: A user with this email already exists.");
    }
    // Auth: Only admin or the user themselves can update
    if (loggedInUser.role !== EROLES.ADMIN && loggedInUser.id !== id) {
        LoggerService.error(`[UsersController] Forbidden: User ${loggedInUser.username} tried to update user ${id}`);
    ...
    if (!updatedUser) {
        LoggerService.error(`[UsersController] Not Found: User ${id} not found or inactive`);
        return res.status(404).send("Not Found: User not found or inactive");
    }
    LoggerService.info(`[UsersController] User ${id} successfully updated by ${loggedInUser.username}`);
    return res.status(200).json(updatedUser);
});
usersController.patch("/:id/role/:role", AuthService.authorize, (req, res) => {
    // 1. Log the incoming request
    LoggerService.info(`[UsersController] PATCH /users/${req.params.id}/role/${req.params.role} called`);
    const loggedInUser = req.user;
    const id = Number(req.params.id);
    ...
    if (loggedInUser.role !== EROLES.ADMIN) {
        LoggerService.error(`[UsersController] Forbidden: User ${loggedInUser.username} is not an Admin`);
    ...
    if (updatedUser === null) {
        LoggerService.error(`[UsersController] Bad Request: User ${id} is not a player and cannot change role`);
    ...
    LoggerService.info(`[UsersController] Role of user ${id} successfully changed to ${newRole} by ${loggedInUser.username}`);
    return res.status(200).json(updatedUser);
});
usersController.patch("/:id/reactivate", AuthService.authorize, (req, res) => {
    // 1. Log the incoming request
    LoggerService.info(`[UsersController] PATCH /users/${req.params.id}/reactivate called`);
    const loggedInUser = req.user;
    const id = Number(req.params.id);
    // 2. Guard: Check if the ID is a valid number
    if (!isNumber(id)) {
    ...
    if (!isReactivated) {
        LoggerService.error(`[UsersController] Not Found/Conflict: User ${id} already active or not found`);
        return res.status(404).send("User can't be reactivated : already activated or user not found");
    }
    // 7. Happy Path: Success
    LoggerService.info(`[UsersController] User ${id} successfully reactivated by ${loggedInUser.username}`);
