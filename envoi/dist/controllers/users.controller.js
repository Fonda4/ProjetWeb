...
usersController.post("/", (req, res) => {
    ...
    if (!UsersService.isValidEmail(userData.email)) {
        ...
        return res
            .status(400)
            .send("Bad Request: The email must contain an '@', a '.' and be at least 5 characters long.");
    }
...
usersController.get("/:id", AuthService.authorize, (req, res) => {
    ...
    if (loggedInUser.role === EROLES.ADMIN) {
        LoggerService.info(`[UsersController] Admin ${loggedInUser.username} retrieved full data for user ${id}`);
        return res.status(200).json(UsersMapper.toDTO(targetUserDBO));
    }
    else if (loggedInUser.id === id) {
        LoggerService.info(`[UsersController] User ${loggedInUser.username} retrieved their own short profile`);
        return res.status(200).json(UsersMapper.toShortDTO(targetUserDBO));
    }
    else {
        LoggerService.error(`[UsersController] Forbidden: ${loggedInUser.username} tried to access user ${id}'s profile`);
        return res
            .status(403)
...
usersController.put("/:id", AuthService.authorize, (req, res) => {
    ...
    if (loggedInUser.role !== EROLES.ADMIN && loggedInUser.id !== id) {
        ...
        return res
            .status(403)
            .send("Authenticated user is not an admin and tries to update another user");
    }
