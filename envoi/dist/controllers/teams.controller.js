 ...
 */
teamsController.get("/own", AuthService.authorize, (req, res) => {
    // 1. Log the incoming request
    LoggerService.info("[TeamsController] GET /teams/own called");
    const loggedInUser = req.user;
    // 2. Guard: Check if the user is properly authenticated
    if (!loggedInUser) {
        LoggerService.error("[TeamsController] Unauthorized: Missing user information");
    ...
    LoggerService.info(`[TeamsController] Teams retrieved successfully for user ${loggedInUser.username}`);
 ...
 * Auth: Not required
 */
teamsController.get("/:id", (req, res) => {
    // 1. Log the incoming request
    LoggerService.info(`[TeamsController] GET /teams/${req.params.id} called`);
    const id = Number(req.params.id);
    // 2. Guard: Check if the ID is a valid number
    if (!isNumber(id)) {
        LoggerService.error(`[TeamsController] Bad Request: Invalid ID provided: ${req.params.id}`);
    ...
    if (!team) {
        LoggerService.error(`[TeamsController] Not Found: Team ${id} does not exist`);
 ...
 */
teamsController.post("/", AuthService.authorize, (req, res) => {
    ...
    if (loggedInUser.role !== EROLES.TRAINER) {
        LoggerService.error(`[TeamsController] Forbidden: User ${loggedInUser.username} is not a trainer`);
    ...
    LoggerService.info(`[TeamsController] Team created successfully by ${loggedInUser.username}`);
 ...
 * Auth: REQUIRED (Role 'trainer' only)
 */
teamsController.put("/:id", AuthService.authorize, (req, res) => {
    LoggerService.info(`[TeamsController] PUT /teams/${req.params.id} called`);
    const loggedInUser = req.user;
    // Check authentication
    if (!loggedInUser) {
        LoggerService.error("[TeamsController] Unauthenticated user attempted to update a team");
        return res.status(401).send("Unauthenticated user");
    }
    ...
    if (id !== teamData.id) {
        LoggerService.error("[TeamsController] Bad Request: Path ID and Body ID mismatch");
    ...
    // Happy Path: Success
    LoggerService.info(`[TeamsController] Team ${id} successfully updated by ${loggedInUser.username}`);
 ...
 * Auth: REQUIRED (Any role)
 */
teamsController.patch("/:id/join", AuthService.authorize, (req, res) => {
    LoggerService.info(`[TeamsController] PATCH /teams/${req.params.id}/join called`);
    const loggedInUser = req.user;
    // Check authentication
    if (!loggedInUser) {
        LoggerService.error("[TeamsController] Unauthenticated user attempted to join a team");
        return res.status(401).send("Unauthenticated user");
    }
    const id = Number(req.params.id);
    // Guard: Verify that the ID is a number
    ...
    if (updatedTeam === null) {
        LoggerService.error(`[TeamsController] Bad Request: User ${loggedInUser.username} is already in team ${id}`);
        return res.status(400).send("Bad Request: User already in the team");
    }
    // Happy Path: Success
    LoggerService.info(`[TeamsController] User ${loggedInUser.username} successfully joined team ${id}`);
    res.status(200).json(updatedTeam);
});
/**
 ...
 * Auth: REQUIRED (Any role)
 */
teamsController.patch("/:id/leave", AuthService.authorize, (req, res) => {
    LoggerService.info(`[TeamsController] PATCH /teams/${req.params.id}/leave called`);
    const loggedInUser = req.user;
    // Check authentication
    if (!loggedInUser) {
        LoggerService.error("[TeamsController] Unauthenticated user attempted to leave a team");
        return res.status(401).send("Unauthenticated user");
    }
    const id = Number(req.params.id);
    // Guard: Verify that the ID is a number
    ...
    if (updatedTeam === undefined || updatedTeam === null) {
        LoggerService.error(`[TeamsController] Not Found: Team ${id} does not exist or user ${loggedInUser.username} is not a member`);
        return res
            .status(404)
            .send("Not Found: Team does not exist or user not in team");
    }
    // Happy Path: Success
    LoggerService.info(`[TeamsController] User ${loggedInUser.username} successfully left team ${id}`);
    res.status(200).json(updatedTeam);
});
