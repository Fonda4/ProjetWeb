...
authController.post('/login', (req, res) => {
    ...
    if (!isUserLoginDTO(loginData)) {
        LoggerService.error("[AuthController] Bad Request: Missing username or password");
    ...
    if (!authResult) {
        LoggerService.error(`[AuthController] Unauthorized: Invalid credentials for user ${loginData.username}`);
        return res.status(401).send("Unauthorized: Invalid credentials");
    }
    // 4. Happy Path : Success ! Return the object with the token
    LoggerService.info(`[AuthController] User ${loginData.username} successfully authenticated`);
