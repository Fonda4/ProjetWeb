...
export class UsersService {
    ...
    static getByUsername(username) {
        const usersDB = this.readUsersDB();
    ...
    /**
     *validateUser
     *Validates a user's password against their hash
     */
    static validateUser(password, passwordHash) {
    ...
    static reactivate(id) {
        ...
        if (index === -1 || dbos[index].status === EUserStatus.ACTIVE)
            return false;
    ...
    static checkCredentials(loginData) {
        const dbos = this.readUsersDB();
        for (const dbo of dbos) {
            if (dbo.username === loginData.username &&
                dbo.status === EUserStatus.ACTIVE) {
...
UsersService.dbPath = "./data/users.json";
