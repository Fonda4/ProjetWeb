...
export class GamesService {
  ...
  private static getNewID(dbos: GameDBO[]): number {
    let maxId = 0;
  ...
  static update(id: number, gameData: GameDTO): GameDTO | undefined | null {
    ...
    if (
      currentGame.status === EGameStatus.FINISHED ||
    ...
    if (currentGame.status === EGameStatus.STARTED) {
      if (
        gameData.fieldId !== currentGame.field_id ||
...
dbos[index].name = gameData.name ?? currentGame.name;
    ...
    if (
      currentGame.status === EGameStatus.CREATED ||
  ...
  static setStatus(
    ...
    if (
      currentStatus === EGameStatus.CREATED &&
      newStatus === EGameStatus.CANCELLED
    )
      isValidTransition = true;
    if (
      currentStatus === EGameStatus.SCHEDULED &&
      newStatus === EGameStatus.CANCELLED
    )
      isValidTransition = true;
    if (
      currentStatus === EGameStatus.STARTED &&
